...
# Currency formatting helper consistent with existing dashboard (₹Indian style)
js = """
// ═══════════════════════════════════════════════════════════════
...
# Insert the JS before the closing </script> of the main script block.
# The file has many script blocks; find the last big one — safer: insert
# right before the marker 'function loadPayments' block? loadPayments is
# defined inside a <script>; placing new code before it keeps scope intact.
marker = 'async function loadPayments() {'
if marker not in html:
    print('JS MARKER NOT FOUND'); sys.exit(1)
html = html.replace(marker, js + '\n' + marker, 1)

with open(path, 'w') as f:
    f.write(html)
print('JS patched OK; length:', len(html))
