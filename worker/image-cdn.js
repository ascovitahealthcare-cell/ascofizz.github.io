 ...
 * ──────────────────────────────────────────────────────────────────────────
 * Edge image proxy: every site image is now delivered from ascofizz.com
...
// The Supabase project that owns the product-images bucket.
//
// The previous operator's project ref used to be hard-coded here, which
// meant a fork silently proxied images out of someone else's storage.
// Set STORAGE_ORIGIN as a Worker environment variable instead; the
// constant below is only the fallback used when it is unset, and it is
// deliberately not a real project.
export const SUPABASE_STORAGE_ORIGIN = 'https://YOUR-PROJECT-REF.supabase.co';
...
export async function handleCdnRequest(request) {
  ...
  // Cloudflare Cache key includes the full ascofizz.com URL; ?w= and similar
